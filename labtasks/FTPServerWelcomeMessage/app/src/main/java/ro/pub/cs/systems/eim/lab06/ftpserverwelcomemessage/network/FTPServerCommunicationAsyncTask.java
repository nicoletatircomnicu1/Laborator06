...
public class FTPServerCommunicationAsyncTask extends AsyncTask<String, String, Void> {
    ...
    protected Void doInBackground(String... params) {
        ...
        try {
            ...
            String start_code = bufferedReader.readLine();
            if (start_code.startsWith(Constants.FTP_MULTILINE_START_CODE)) {
                String info_line = bufferedReader.readLine();
                while(!info_line.equals(Constants.FTP_MULTILINE_END_CODE1) && !info_line.startsWith(Constants.FTP_MULTILINE_END_CODE2)) {
                    publishProgress(info_line);
                    info_line = bufferedReader.readLine();
                }
            }
            socket.close();
